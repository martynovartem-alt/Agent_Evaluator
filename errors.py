...
class NetworkError(ApiError):
    ...
    # cert errors happen ON the VPN, so the default VPN hint would mislead — used by oai.py
    cert_fix = ("the endpoint presents a self-signed certificate — set insecure = true in the "
                "role's section of agents.toml (curl -k equivalent); proper fix: get the bank "
                "CA cert and set SSL_CERT_FILE=/path/to/alfa_ca.pem in .env")
