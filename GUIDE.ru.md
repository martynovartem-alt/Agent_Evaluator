...
поток: **быстрая проверка, потом полный прогон**. `eval_fast.py` оценивает 20 случайных диалогов и
...
| `HTTP 400 … "HAS_PERSONAL_DATA"` | DLP Sandbox сканирует запросы и отклоняет персональные данные (имена, карты, почта в реальных диалогах) | `sanitize = true` в секции роли (в поставке включено) — маскировка через `privacy.py` + один повтор со строгой маскировкой; если строка всё равно падает — найдите триггер в диалоге и расширьте паттерны в `privacy.py` |
